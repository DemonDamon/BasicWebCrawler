...
class MockPage:
    url: str = ""
    _html: str = ""
    _hrefs: list[str] = field(default_factory=list)
    ...
    def goto(self, url: str, *, wait_until: str = "domcontentloaded", timeout: int = 30000):
        ...
        if "weixin.sogou.com/weixin" in url:
            self.url = url
            self._html = "<html><body><div class='news-list'></div></body></html>"
            return None
        if "weixin.sogou.com/link" in url:
            self.url = "https://mp.weixin.qq.com/s/ABC123xyz"
            self._html = "<html><body>article</body></html>"
            return None
        self.url = url
        return None
...
def test_build_sogou_search_url_encodes_query() -> None:
...
def test_follow_redirect_links_collects_wechat_urls() -> None:
    ...
    assert links[0].source == "sogou_playwright"
...
def test_discover_from_search_page_with_mock_hrefs(monkeypatch) -> None:
    page = MockPage(
        _hrefs=[
    ...
    result = discover_from_search_page(page, "机器之心")
    get_settings.cache_clear()
...
def test_provider_discover_for_org(db_session, monkeypatch) -> None:
    ...
    page = MockPage(
        _hrefs=["https://weixin.sogou.com/link?url=only"],
    )
