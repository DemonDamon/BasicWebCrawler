...
│  /api/accounts/   ← 账号管理 + RSSHub 路由配置             │
│  /admin           ← 浏览器查看入库结果                      │
│  /docs            ← Swagger 在线接口文档                    │
│                                                             │
│  候选池（ArticleCandidate）                                 │
│    ↑ rss_poller / sogou_poller 发现新 URL 入队             │
│    ↑ 插件/命令行手动入队                                    │
│    ↓ fetch_worker / 插件自动采集 消费 → 入库               │
└─────────────────────────────────────────────────────────────┘
         ↑                            │
RSSHub（本地 Docker :1200）           ▼
freewechat / wechat2rss          wechat_collector.db（SQLite）
RSS feed 定期拉取                 或远程 PostgreSQL（.env 配置）
```
...
| **RSS 巡检**（rss_poller）          | 自动发现各号新文章（需 RSSHub） | ❌ 不需要      | 全自动（freewechat 常失效） |
| **Playwright 搜狗**（sogou_poller） | 真实浏览器从搜狗搜索发现 URL    | ❌ 不需要      | 全自动（推荐）             |


> 路径互补：Playwright 搜狗 / RSS 负责发现，Worker / 插件负责处理；需要登录态的内容仍靠插件。
...
当 RSSHub / freewechat 不可用，或搜狗 HTTP 抓取被反爬拦截时，使用 **Playwright 真实浏览器**从搜狗微信搜索跟随跳转，解析真实的 `mp.weixin.qq.com/s/...` URL 并入候选池。
...
| `目标号最近=YYYY-MM-DD（约 N 天前）` | 搜狗里该号最新一篇也超出窗口 → 需调大天数或换 RSS/插件 |
...
**搜狗索引滞后**的号（如果 `目标号最近` 已是 2021 年）无法靠调天数解决，需 freewechat / 插件 / RSS 补采。
...
- 搜狗收录有延迟，刚发布的文章可能暂时搜不到；InfoQ 等号需等收录或配合插件/RSS 补采。
